...
const levelsArray = ['LEVEL_UNSPECIFIED', 'TRACE', 'DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'] as (keyof typeof LogLevel_Level)[];

test('should return default value LEVEL_UNSPECIFIED when key is not set', () => {
  expect(parseLogOptionsMinLevel(getInput('revision-log-options-min-level', {required: false, trimWhitespace: true}))).toEqual(
...
test.each(levelsArray)('should return correct enum for LogLevel = %s', (level: keyof typeof LogLevel_Level) => {
  ...
  const result = parseLogOptionsMinLevel(getInput('revision-log-options-min-level', {required: false, trimWhitespace: true}));
