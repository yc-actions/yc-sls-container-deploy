...
1.  **Input Configuration**: The action's configuration in `action.yml` will be extended to accept a multi-line
    `secrets` input.
2.  **Parsing**: A dedicated parser will process the `secrets` input string. Each line will represent a single secret
    and its mapping to an environment variable. The parser will be located in `src/parse/lockboxVariables.ts`.
3.  **'Latest' Version Resolution**: A helper function, `resolveLatestLockboxVersions`, will be implemented in
    ...
    secrets that use it.
4.  **API Request**: The resolved secrets will be included in the `CreateFunctionVersionRequest` sent to the Yandex
    Cloud API.
